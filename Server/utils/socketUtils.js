...
export const initSocket = (io) => {
  ...
  const hasOtherSocketForUser = (userId, currentSocketId) => {
    for (const s of io.of("/").sockets.values()) {
      if (s.id === currentSocketId) continue;
      if (String(s.userId) === String(userId)) return true;
    }
    return false;
  };

  const hasAnySocketForUser = (userId) => {
    for (const s of io.of("/").sockets.values()) {
      if (String(s.userId) === String(userId)) return true;
    }
    return false;
  ...
  io.on("connection", async (socket) => {
    try {
      ...
      // Send current online users snapshot to newly connected socket
      socket.emit("users:online", { userIds: getConnectedUserIds() });

      // Broadcast presence only when user's first active socket connects
      if (!hasOtherSocketForUser(socket.userId, socket.id)) {
      ...
      socket.on("call-initiate", async ({ to, conversationId, fromName, callType }) => {
        try {
          ...
          // Check whether callee has at least one active socket
          const calleeOnline = hasAnySocketForUser(to);
          if (!calleeOnline) {
      ...
      socket.on("call-user", async ({ to, conversationId, offer, callId }) => {
        try {
          ...
          // Prevent unrelated concurrent calls in the same conversation,
          // but allow the same caller/callee flow after call-initiate.
          const existingCall = activeCalls.get(String(conversationId));
          const isSameCallFlow =
            existingCall &&
            String(existingCall.callerId) === String(socket.userId) &&
            String(existingCall.calleeId) === String(to);

          if (existingCall && !isSameCallFlow) {
      ...
      socket.on("disconnect", () => {
        try {
          ...
          const stillOnline = hasAnySocketForUser(socket.userId);
          if (!stillOnline) {
...
export function emitMessageToRoom(conversationId, payload) {
  ...
  try {
    ...
    ioRef.to(convId).emit("message:new", payload);

    // Also notify each participant's personal room so chats update even
    // when the receiver has not joined the conversation room yet.
    Conversation.findById(convId)
      ...
      .then((conv) => {
        ...
        for (const participantId of participants) {
          ioRef.to(String(participantId)).emit("notification:new_message", {
            conversationId: convId,
            message: payload,
          });
        }
      ...
      .catch((err) => {
        console.error("emitMessageToRoom participant notify error:", err?.message || err);
      });
  } catch (err) {
